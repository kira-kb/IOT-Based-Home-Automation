...
if (isset($_GET['token'])) {
    if (!empty($_GET['token'])) {
        ...
        if (!empty($_SESSION[md5($getToken)])) {
            $sessionToken = $_SESSION[md5($getToken)];

            if (time() >= $sessionToken['endTime']) {
                unset($_SESSION[md5($getToken)]);
                $response = '<p class="error">Reset token expired, please try again!</p>';
                $isValidToken = false;
            } else {
                $isValidToken = true;
            }
        } else {
            $response = '<p class="error">Invalid token!</p>';
        }
    }
}
// echo '<br>';
...
if (isset($_POST['sendEmail'])) {
    if (empty($_POST['email'])) {
        $response = '<p class="fillInfo">Please provide an email address!</p>';
    } else {
        require 'sendMail.php';
        require 'connectDB.php';

        $emailToken = randStr(50);
        $email = $_POST['email'];

        // echo $emailToken;
        // echo '<br>';

        // Check if the email exists in customers and/or employees tables
        $sqlCustomers = "SELECT `email` FROM `customers` WHERE `email` = '$email'";
        $resCustomers = $con->query($sqlCustomers);

        $sqlEmployees = "SELECT `email`, `role` FROM `employees` WHERE `email` = '$email'";
        $resEmployees = $con->query($sqlEmployees);

        $customerEmail = $resCustomers->num_rows > 0 ? $resCustomers->fetch_assoc()['email'] : null;
        $employeeEmail = $resEmployees->num_rows > 0 ? $resEmployees->fetch_assoc() : null;

        // var_dump($customerEmail);
        // var_dump($employeeEmail);
        // echo "<br>";
        // die($customerEmail || $employeeEmail);
        if ($customerEmail || $employeeEmail) {

            $template = '<!DOCTYPE html>
            <html lang="en" style="display: flex; justify-content: center; width: 100%; font-family: sans-serif;">
            <head>
            <style>
                body { margin: 0; padding: 0; width: 100%; color: #333;}
                .container { width: 60%; margin: 0 auto; background-color: #eee; padding: 15px 0; }
                .header { text-align: center; padding: 10px; background-color: #333; color: #eee; }
                .header h2 { margin: 0; }
                .icon { margin-bottom: 10px; }
                .note { font-size: 18px; }
                .description { font-size: 15px; line-height: 1.5; padding: 0 15px;}
                .btn { display: inline-block; background-color: #333; color: #eee; padding: 5px 20px; text-decoration: none; margin-top: 10px; }
                .excuse { font-size: 12px; font-style: italic; }
...
if (isset($_POST['resetPassword'])) {
    if (empty($_POST['password']) || empty($_POST['confirmPassword'])) {
        $response = '<p class="fillInfo">Please provide both fields!</p>';
    } elseif ($_POST['password'] != $_POST['confirmPassword']) {
        $response = '<p class="error">Password and confirm password are not the same!</p>';
    } else {
        require 'connectDB.php';

        $newPassword = md5($_POST['password']);
        $email = $_SESSION[md5($_GET['token'])]['email'];

        $role = '';
        if (isset($_POST['role'])) {
            $role = $_POST['role'];
        } else {
            // var_dump($_SESSION[md5($_GET['token'])]['isCustomer']);
            if ($_SESSION[md5($_GET['token'])]['isCustomer']) {
                $role = 'customer';
            } elseif ($_SESSION[md5($_GET['token'])]['isEmployee']) {
                // echo "role";
                $role = 'employee';
            }
        }

        // Update the password in the appropriate table based on the selected role
        $sql = '';
        if ($role === 'customer') {
        ...
        if ($con->query($sql)) {
            $response = '<p class="success">Password has been reset successfully! <a href="./home/index.php">Click here to login</a></p>';
            unset($_SESSION[md5($_GET['token'])]); // Remove the token session after successful reset
            $isValidToken = false;
        } else {
            $response = '<p class="error">Failed to reset password! Please try again later!</p>';
        }
    }
}
?>
...
<body>
    <form action="" method="post">
        <div class="info">Send an email to reset your password</div>
        <?php if (!isset($isValidToken) || !$isValidToken) { ?>
            ...
            ?>
            <button type="submit" name="sendEmail">Send reset password via email</button>
        <?php } else { ?>
            <label for="password">New Password</label>
            <input type="password" name="password" placeholder="New Password" />
            <label for="confirmPassword">Confirm Password</label>
            <input type="password" name="confirmPassword" placeholder="Confirm Password" />
            <?php if ($_SESSION[md5($_GET['token'])]['isCustomer'] && $_SESSION[md5($_GET['token'])]['isEmployee']) { ?>
                <fieldset>
                    <legend class="accInfo" for="role">Select account type:</legend>
                    <leg class="roleContainer">
                        <label class="roleC" for="customer">Customer</label>
                        <input class="roleR" type="radio" id="customer" name="role" value="customer" required />
                        <label class="roleE" for="employee">Employee (Role:
                            <?php echo $_SESSION[md5($_GET['token'])]['employeeRole']; ?>)</label>
                        <input type="radio" class="roleR" id="employee" name="role" value="employee" required />
                    </leg>
                </fieldset>
            <?php } ?>
            <?php if ($response) {
